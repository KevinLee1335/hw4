...
```{r}
# Write your code here
cute_cute <- readr::read_csv("week3Rclass.csv")
...
請修改以下 data frame 的資料：
  ...
  - gender："M" 改為 1、"F" 改為2。其它的則改為 0。注意：gender 資料類型需為 `integer`。(Hint: 注意 gender 原本的資料類型是沒有教過的 )
factor
```{r}
# Data 
Q_Q = data.frame(
...
Q_Q$gender <- as.numeric(Q_Q$gender)
...
```{r}
# Write your code here
cute_cute <- cbind(cute_cute, favorite_word, c(1:85))
